...
public class TeacherLoginAction extends ActionSupport implements SessionAware {
    	...
    	Teachers teacher = new LoginDaoImpl().findByCode(username);
            if(teacher!=null&&teacher.getTeachersPasswd().equals(password)){
        		...
        		if(session.get("teacher")==null){
        			session.put("teacher", teacher);/*若登陆成功则将该user记录在当前会话中*/
        			System.out.println(
        			...
        			return "success";
        		}else if(username.equals(((Teachers) session.get("teacher")).getTeachersId())){
        			System.out.println("你已登录当前会话");
        			return "success";
        		}else{
        			System.out.println("先退出当前用户"+((Teachers) session.get("teacher")).getTeachersName()+"才能登录新用户");
        			return "fail";
        		}
            }else{
	...
	public String logout(){
		...
		System.out.print("注销当前会话（"+((Teachers) session.get("teacher")).getTeachersName()+"）");
